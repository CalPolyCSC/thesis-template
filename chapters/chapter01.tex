...
\section{Nomenclature Usage} \label{sec:NomenclatureUsage}
    ...
    All of the nomenclature information is contained within the \textbf{nomenclature.tex} file in the \textbf{frontmatter} directory.
...
\section{Citations} \label{sec:Citations}
    ...
    The bibliography data is located in the bib-file, \textbf{references.bib}, located in the \textbf{bibliography} directory.
    ...
    Many publishers offer the ability to export a citation in bibtex format, and this can easily be included into the \textbf{references.bib} file (sometimes without any modifications).
    There are also more sophistical reference management systems, such as \href{https://www.zotero.org/}{Zotero} and \href{https://www.mendeley.com}{Mandeley}, that can export citations.
    No matter where the citation source was generated, it will probably take some fine tuning to make the cited work correctly formatted.

    The other file in the \textbf{bibliography} directory is the \textbf{bib\_info.tex} file.
    ...
    To cite an entry can be done using the \textbf{cite} command, such as \cite{cpthesis_github}.
    ...
    To refer to the author(s) and cite the \textbf{textcite} command can be used as in     \textcite{dirac1981}.
...
\section{Code and Code Listings} \label{sec:CodeAndCodeListings}
    ...
    Ideally, it would be nice to differentiate inline code so that it is clear that \verb|incmatrix| is a function defined in code.
    The following listing, \cref{lst:OverleafCode}, shows an example python code originally from Overleaf.\footnote{Complete example can be found at \url{https://www.overleaf.com/learn/latex/Code_listing}}
    This code is in a floating environment (note the usage of \textbf{float} option).
    Without that option the code listing will appear exactly in the text where the code listing is.

    \begin{lstlisting}[float, caption=Example from Overleaf demonstrating python code., label=lst:OverleafCode]
import numpy as np
